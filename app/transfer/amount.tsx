  ...
  return (
    <SafeAreaView className="flex-1" style={{ backgroundColor: colors.background }} edges={['top']}>
      ...
      <View className="flex-row items-center px-6 py-4">
        ...
        <Text className="flex-1 text-center text-xl font-bold pr-10" style={{ color: colors.text }}>Enter Amount</Text>
      </View>

      <ScrollView className="flex-1 px-6" showsVerticalScrollIndicator={false}>
        {/* Beneficiary Card ... */}
        <View className="p-5 rounded-[32px] flex-row items-center mt-6 shadow-sm border" style={{ backgroundColor: colors.surface, borderColor: colors.cardBorder }}>
          <View className="w-14 h-14 rounded-full items-center justify-center mr-4 overflow-hidden" style={{ backgroundColor: colors.primary }}>
          ...
          <View className="flex-1">
            <Text className="font-bold text-base mb-1" style={{ color: colors.text }}>{params.name}</Text>
            <Text className="text-xs" style={{ color: colors.textSecondary }}>{params.account} - {params.bank}</Text>
          </View>
        </View>

        {/* Amount Display */}
        <View className="p-8 rounded-[40px] mt-4 items-center shadow-sm border" style={{ backgroundColor: colors.surface, borderColor: colors.cardBorder }}>
          <Text className="text-sm font-medium mb-4" style={{ color: colors.textSecondary }}>Enter Amount</Text>
          <Text className="text-5xl font-extrabold mb-4" style={{ color: colors.text }}>{amount}</Text>
          <Text className="text-sm" style={{ color: colors.textSecondary }}>Available balance : ₦{balance}</Text>
        </View>

        {/* Narration */}
        <View className="mt-6">
          <TextInput
            className="w-full h-14 border rounded-2xl px-5 font-medium" style={{ borderColor: colors.cardBorder, color: colors.text, backgroundColor: colors.surface }}
        ...
        <View className="flex-row justify-between mt-6">
          {quickAmounts.map((val) => (
            <TouchableOpacity 
              ...
              className={`flex-1 mx-1 py-4 rounded-2xl items-center border ${amount === val ? 'border-[#5154F4]' : 'border-transparent'}`}
              style={{ backgroundColor: amount === val ? colors.surface : colors.surface }}
            >
              <Text className={`font-bold`} style={{ color: amount === val ? colors.primary : colors.text }}>{val}</Text>
            </TouchableOpacity>
          ))}
        </View>
        ...
        <TouchableOpacity 
          ...
          className="mt-8 py-5 rounded-[28px] shadow-lg shadow-indigo-100 mb-6" style={{ backgroundColor: colors.primary }}
        >
