      ...
      <ScrollView className="flex-1 px-5" showsVerticalScrollIndicator={false}>
        ...
        <View className="bg-white p-8 rounded-[40px] mt-6 border border-gray-50 shadow-sm">
          <View className="items-center mb-8">
            ...
            <Text className="text-[#1F2C37] text-2xl font-extrabold mb-1">
              {goal.name}
            </Text>
            <Text className="text-[#9DA3B6] text-xs font-bold uppercase tracking-widest">
              {goal.description}
            </Text>
          </View>

          <View className="flex-row justify-between mb-2">
            <Text className="text-[#9DA3B6] font-bold text-xs uppercase tracking-widest">
              Progress
            </Text>
          ...
          <View className="flex-row justify-between items-center bg-[#F8F9FB] p-6 rounded-3xl">
            <View>
              <Text className="text-[#9DA3B6] text-[10px] font-bold uppercase tracking-widest mb-1">
                Saved
              </Text>
              <Text className="text-[#1F2C37] text-xl font-black">
        ...
        <View className="mt-10 mb-10">
          <Text className="text-[#1F2C37] text-lg font-bold mb-6">
            Recent Activities
          </Text>
          ...
          {isLoadingHistory ? (
            ...
            history?.map((item: any) => (
              <View
                key={item.id}
                className="bg-white p-5 rounded-3xl flex-row items-center mb-3 border border-gray-50"
              >
                <View
                  className={`w-10 h-10 ${item.type === "FUND" ? "bg-green-50" : "bg-red-50"} rounded-2xl items-center justify-center mr-4`}
                >
                  <Feather
                    name={
                      item.type === "FUND"
                    ...
                    size={18}
                    color={item.type === "FUND" ? "#22C55E" : "#EF4444"}
                  />
                </View>
                <View className="flex-1">
                  <Text className="text-[#1F2C37] font-bold text-sm mb-0.5">
                ...
                <Text
                  className={`font-bold text-sm ${item.type === "FUND" ? "text-green-600" : "text-red-500"}`}
                >
                  {item.type === "FUND" ? "+" : "-"}₦
            ...
            >
              ...
              <View className="bg-indigo-50/50 rounded-3xl p-6 border border-indigo-100 mb-10">
                <View className="flex-row items-center justify-between mb-4">
                  <Text className="text-[#9DA3B6] text-[10px] font-bold uppercase tracking-widest">
                    Available to withdraw
                  </Text>
                  <Text className="text-[#5154F4] text-sm font-black">
                ...
                <View className="bg-white border border-indigo-200 h-[72px] rounded-2xl px-5 flex-row items-center">
                  <Text className="text-[#1F2C37] text-2xl font-black mr-2">
                    ₦
                  </Text>
                  <TextInput
